    ...
    def _verify_rs(self, header_b64: str, payload_b64: str,
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(mode='wb', suffix='.sig', delete=False) as f:
                f.write(_b64decode(signature_b64))
                sig_path = f.name

            # OpenSSL 验证
            message = f"{header_b64}.{payload_b64}".encode()
            with tempfile.NamedTemporaryFile(mode='wb', suffix='.msg', delete=False) as f:
                f.write(message)
                msg_path = f.name
            ...
            for p in [key_path, sig_path, msg_path]:
                try: os.unlink(p)
                except OSError: pass
    ...
    def verify_mtls(self, client_cert_pem: str) -> Optional[dict]:
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(mode='w', suffix='.pem', delete=False) as f:
                ...
                cert_path = f.name
            result = subprocess.run(
                ["openssl", "verify", "-CAfile", self.mtls_ca_cert, cert_path],
                capture_output=True, timeout=5
            )
